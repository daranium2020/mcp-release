...
```bash
mcp-release --help
mcp-release --version          # must print 0.2.1
...
# Expected: PASS with INIT_OK and one tool (echo); report shows transportType: stdio, executionEnvironment: cli
...
# Expected: WARNING with STDIO_UNEXPECTED_OUTPUT and remediation hint

# Verify JSON output omits transport field for stdio
...
```bash
# HTTP report should include all new fields
...
# Expected: {"transportType":"http","executionEnvironment":"cli","mcpReleaseVersion":"0.2.1","startedAt":"<ISO timestamp>"}
```
...
5. **Publish the CLI to npm:**
   ...
   npm publish
   ```
   Confirm the published version is `0.2.1` at https://www.npmjs.com/package/@mcp-release/cli.
...
7. Verify the new CLI version:
   ```bash
   npm install -g @mcp-release/cli   # installs 0.2.1
   mcp-release --version             # should print 0.2.1
   mcp-release check https://mcp-release-fixture.vercel.app/mcp
...
- If a packaging issue is found after publish, release a patch version (`0.2.2`) with the fix.
- If a critical security issue is found, deprecate the affected version with `npm deprecate @mcp-release/cli@0.2.1 "reason"` and publish a patched version immediately.
